...
def sign_up():
    if request.method == "POST":
        ...
        if (user):
            ...
            flash('Please choose another first name.', category='error') 
        elif len(email) < 4:
            ...
            new_user = User(email=email, first_name=first_name, address=address, post_code=post_code, password=generate_password_hash(
